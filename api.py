...
def configure_ngrok():
    ngrok_token = "2pQloMrae22UQ2CXM45cnXOv6tj_7zrEicz3dzqAyVnAwY4pM"
    try:
...
def update_soul_txt(public_url):
    with open("anony5.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in anony5.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("anony5.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated anony5.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update anony5.txt on VPS: {str(e)}")
